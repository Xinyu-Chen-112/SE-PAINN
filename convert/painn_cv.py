    ...
    for i in range(len(structure)):
        ...
        for j in range(len(nbrs)):
            if ceng[i] == ceng[nbrs[j][2]] and n_che < args.che_max_num_nbrs:
                ...
                che_info[1].append((np.array(nbrs[j].coords) - np.array(structure[i].coords)).tolist())
                che_info[2].append(nbrs[j][1])
                n_che += 1
            elif ceng[i] != ceng[nbrs[j][2]] and n_vdw < args.vdw_max_num_nbrs:
                vdw_info[0].append([i, nbrs[j][2]])
                vdw_info[1].append((np.array(nbrs[j].coords) - np.array(structure[i].coords)).tolist())
                vdw_info[2].append(nbrs[j][1])
                n_vdw += 1
        information[2].append(che_info)
        information[3].append(vdw_info)
