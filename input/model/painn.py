...
class PainnMessage(nn.Module):
    def __init__(self,
        ...
        self.filter_layer = nn.Sequential(nn.Linear(edge_embedding_size, hidden_size),
                                          nn.PReLU(),
                                          nn.Linear(hidden_size, 3 * hidden_size),
                                          nn.PReLU())
...
class Model(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        rbf_dist = torch.where((edge_dist < self.cutoff).unsqueeze(-1),
                                torch.sin(edge_dist.unsqueeze(-1)
                                          * (torch.arange(self.edge_embedding_size, device=edge_dist.device) + 1)
                                          * torch.pi
                                          / self.cutoff)
                                / edge_dist.unsqueeze(-1),
                                torch.tensor(0.0, device=edge_dist.device, dtype=edge_dist.dtype))
        rbf_dist = rbf_dist * 0.5 * (torch.cos(torch.pi * edge_dist / self.cutoff) + 1).unsqueeze(-1)
